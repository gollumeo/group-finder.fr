<template>
    <div :id="id" tabindex="-1" aria-hidden="true"
         class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-full bg-white/10">
        <slot name="wrapper">
            <div class="relative p-4 w-full flex items-center justify-center"
                ...
                <!-- Modal content -->
                <div class="relative bg-indigo-50 rounded-lg shadow dark:bg-slate-700 flex flex-col w-full"
                     :class="fullHeight ? 'h-full' : 'max-h-full'">
                    <!-- Modal header -->
                    <slot name="header">
                        <div
                            class="flex items-center justify-between p-4 md:p-5 border-b border-b-indigo-800 rounded-t dark:border-slate-600">
                            <slot name="title">
                            ...
                            <button type="button" @click="show = false; $emit('close')"
                                    class="bg-transparent hover:bg-indigo-200 text-indigo-500 hover:text-indigo-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                    ...
                    <div
                        ...
                        <slot name="footer">
                            <CancelButton @click="show = false; $emit('close')" innerText="Fermer" :hasIcon="false" />
                            <ValidateButton :hasIcon="false" @click="show = false; $emit('validate')" />
                        </slot>
                    </div>
...
export default {
    name: 'Modal',
    components: {CancelButton, ValidateButton},
    props: {
        modelValue: {
            type: Boolean,
            default: false,
